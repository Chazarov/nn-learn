...
def _get_adjustments() -> List[List[List[float]]]:
...
def test_apply_adjustiments():
    ...
    [
        ...
        [
            [-0.18873, 0.62055, 0.52453]
        ]
    ]

    adjs = _get_adjustments()

    result = Perceptron(weights=WEIGHTS, activations=[Rellu(), Rellu()], layers_count=LAYERS_COUNT)

    apply_adjustments(result.weights, adjs)


    errors:List[Any] = list()
    for q in range(len(EXPECTED_WEIGHTS)):
        for i in range(len(EXPECTED_WEIGHTS[q])):
            for j in range(len(EXPECTED_WEIGHTS[q][i])):
                w = EXPECTED_WEIGHTS[q][i][j]
                # if w != result[q][i][j]:
                if abs(w - result.weights[q][i][j]) > 0.0001:
                    logger.error(f" Test error. incorrect weight  layer {q} | line {i} | column {j}")
                    errors.append({"position":(q, i, j), "expected":w , "received":result.weights[q][i][j]})
